...
class SemifluidGeneratorBlock : MachineBlock() {
    ...
    companion object {
        ...
        fun generateRecipes(exporter: Consumer<RecipeExporter>) {
            ...
            if (geo != Items.AIR && emptyCell != Items.AIR && ironCasing != Items.AIR) {
                ShapedRecipeJsonBuilder.create(RecipeCategory.MISC, SemifluidGeneratorBlock::class.item(), 1)
                    .pattern("CFC")
                    .pattern("FGF")
                    .pattern("CFC")
                    .input('C', ironCasing)
                    .input('F', emptyCell)
                    .input('G', geo)
                    .criterion(hasItem(geo), conditionsFromItem(geo))
                    .offerTo(exporter, SemifluidGeneratorBlock::class.id())
            }
        }
    }
}
