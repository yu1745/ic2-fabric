...
class PumpRenderer(ctx: BlockEntityRendererFactory.Context) :
    ...
    private fun ensureSprites() {
        ...
        fun sprite(path: String) = atlas.getSprite(Identifier(BuildCraftAddon.MOD_ID, path))
    ...
    private fun renderLed(
        ...
        val pm = ctx.positionMatrix
        val nm = ctx.normalMatrix
        ...
        emitLedQuad(buffer, pm, nm, light, overlay,
    ...
    private fun renderTubeBox(
        ...
        val pm = ctx.positionMatrix
        val nm = ctx.normalMatrix
        val u1 = sprite.minU; val u2 = sprite.maxV
        val v1 = sprite.minV; val v2 = sprite.maxU

        val r = 255; val g = 255; val b = 255

        // 4 sides
        emitLedQuad(buffer, pm, nm, 0xF000F0, 0,
    ...
    private fun emitLedQuad(
        buffer: VertexConsumer, pm: Matrix4f, nm: Matrix3f, light: Int, overlay: Int,
        ...
        buffer.vertex(pm, x0.toFloat(), y0.toFloat(), z0.toFloat()).color(r, g, b, 255)
            .texture(u0, v0).overlay(overlay).light(light).normal(nm, nx, ny, nz).next()
        buffer.vertex(pm, x1.toFloat(), y1.toFloat(), z1.toFloat()).color(r, g, b, 255)
            .texture(u1, v1).overlay(overlay).light(light).normal(nm, nx, ny, nz).next()
        buffer.vertex(pm, x2.toFloat(), y2.toFloat(), z2.toFloat()).color(r, g, b, 255)
            .texture(u2, v2).overlay(overlay).light(light).normal(nm, nx, ny, nz).next()
        buffer.vertex(pm, x3.toFloat(), y3.toFloat(), z3.toFloat()).color(r, g, b, 255)
            .texture(u3, v3).overlay(overlay).light(light).normal(nm, nx, ny, nz).next()
    }
