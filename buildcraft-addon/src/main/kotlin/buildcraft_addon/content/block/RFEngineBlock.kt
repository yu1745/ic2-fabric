...
@ModBlock(name = "rf_engine", registerItem = true, tab = CreativeTab.BUILDCRAFT)
class RFEngineBlock : BlockWithEntity(
    Settings.create().strength(5.0f, 10.0f).nonOpaque()
), BlockEntityProvider {

    companion object {
        private val CODEC: MapCodec<RFEngineBlock> = Block.createCodec { RFEngineBlock() }
    ...
    override fun onUse(
        ...
        if (!world.isClient) {
            val be = world.getBlockEntity(pos) as? RFEngineBlockEntity ?: return ActionResult.PASS
    ...
    override fun onPlaced(world: World, pos: BlockPos, state: BlockState, placer: LivingEntity?, itemStack: ItemStack) {
        if (!world.isClient) {
            val be = world.getBlockEntity(pos) as? RFEngineBlockEntity ?: return
    ...
    override fun createBlockEntity(pos: BlockPos, state: BlockState): BlockEntity {
        return RFEngineBlockEntity(pos, state)
    }
    ...
    override fun <T : BlockEntity> getTicker(world: World, state: BlockState, type: BlockEntityType<T>): BlockEntityTicker<T>? {
        return if (world.isClient) {
            BlockEntityTicker { _, _, _, be -> (be as RFEngineBlockEntity).clientTick() }
        } else {
            BlockEntityTicker { _, _, _, be -> (be as RFEngineBlockEntity).serverTick() }
        }
    }
}
