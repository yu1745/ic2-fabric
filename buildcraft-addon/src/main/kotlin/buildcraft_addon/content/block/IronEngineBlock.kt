...
@ModBlock(name = "iron_engine", registerItem = true, tab = CreativeTab.BUILDCRAFT)
class IronEngineBlock : BlockWithEntity(
    Settings.create().strength(5.0f, 10.0f).nonOpaque()
), BlockEntityProvider {

    companion object {
        private val CODEC: MapCodec<IronEngineBlock> = Block.createCodec { IronEngineBlock() }
    ...
    override fun onUse(
        ...
        if (!world.isClient) {
            val be = world.getBlockEntity(pos) as? IronEngineBlockEntity ?: return ActionResult.PASS
    ...
    override fun onPlaced(world: World, pos: BlockPos, state: BlockState, placer: LivingEntity?, itemStack: ItemStack) {
        if (!world.isClient) {
            val be = world.getBlockEntity(pos) as? IronEngineBlockEntity ?: return
    ...
    override fun createBlockEntity(pos: BlockPos, state: BlockState): BlockEntity {
        return IronEngineBlockEntity(pos, state)
    }
    ...
    override fun <T : BlockEntity> getTicker(world: World, state: BlockState, type: BlockEntityType<T>): BlockEntityTicker<T>? {
        return if (world.isClient) {
            BlockEntityTicker { _, _, _, be -> (be as IronEngineBlockEntity).clientTick() }
        } else {
            BlockEntityTicker { _, _, _, be -> (be as IronEngineBlockEntity).serverTick() }
        }
    }
}
