...
从分叉点 `c5e247c` 到 main HEAD 共 **82 个 commit**，**82 个已同步，0 个待同步**。
...
|---|--------|------|:-----------:|------|
| 50 | `84e84c79` | fix: 变压器升压/降压文本互换+ScreenHandler六面朝向修复 | ✅ | 无冲突直接 cherry-pick |
| 51 | `207f0c3d` | feat: 新增 AdjacentEnergyTransferComponent 统一贴脸能量传输 | ✅ | 无冲突直接 cherry-pick |
| 52 | `9cfc66eb` | feat: 全量迁移剩余 26 个机器到 AdjacentEnergyTransferComponent | ✅ | 无冲突直接 cherry-pick |
| 53 | `7bcc2a27` | feat: 所有发电机添加 AdjacentEnergyTransferComponent 支持贴脸能量传输 | ✅ | 无冲突直接 cherry-pick |
| 54 | `e5be3a10` | feat: 飞行改为双击空格触发，移除独立的 Alt+F / Alt+M 按键开关 | ✅ | cherry-pick + 适配 1.21.1 API（Payload 网络、DataComponent NBT）+ datagen 刷新 |
| 55 | `c7a3ae42` | fix: 固体装罐机食物无法放入 + JEI 显示与实际消耗数量不匹配 | ✅ | cherry-pick + 适配 1.21.1 API（DataComponentTypes.FOOD、nutrition）|
| 56 | `3534ffc5` | fix: 流体热力发电机无限免费燃料——tryInsertFuel 在空间不足 1B 时仍注入 | ✅ | 无冲突直接 cherry-pick |
| 57 | `10ca9f12` | feat: 高级太阳能附属机器 Screen 左侧添加发电/输出/耗能状态渲染 | ✅ | 已在 1.21.1 存在（`182ac446`） |
| 58 | `953ba56e` | feat: 添加 enableOvervoltageExplosion 配置项，支持完全关闭超压爆炸 | ✅ | 无冲突直接 cherry-pick |
| 59 | `dd0b29d9` | fix: 修复平面照明器东西方向碰撞箱互换的问题 | ✅ | 无冲突直接 cherry-pick |
| 60 | `66edf7da` | feat: 紫外线灯（UV Lamp）实现——加速作物生长+Jade剩余时间修正 | ✅ | cherry-pick + 适配 1.21.1 API（validateTicker、RecipeExporter、getCodec、getScreenOpeningData、NBT registryLookup、onUse/onUseWithItem 拆分）|
| 61 | `c4379ed2` | fix: 修复紫外线灯模型形状——EAST/WEST 碰撞箱互换 + blockstates 上下旋转角度修正 | ✅ | cherry-pick + 适配 1.21.1 data 目录单复数变化 |
| 62 | `07dda56e` | fix: 拦截 attachLeash 中的 stopRiding，防止区块重载时被拴生物从矿车掉出 | ✅ | cherry-pick + 适配 1.21.1（getHoldingEntity → getLeashHolder）|
| 63 | `c735f845` | fix: 修复紫外线灯上下方向渲染反置——blockstates up/down x-rotation 对齐 ic2_120 日光灯 | ✅ | 无冲突直接 cherry-pick |
