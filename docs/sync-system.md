...
2. 在 BlockEntity 中：`val syncedData = SyncedData(); val sync = XxxSync(syncedData)`，菜单传 `syncedData`，buf 写 `syncedData.size()`，NBT 用 `syncedData.readNbt/writeNbt`。
