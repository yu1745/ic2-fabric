...
说明：
- `WaterGenerator` 等其它流体机器仍可通过自身 `FluidStorage` 交互，但未接入升级驱动的 provider/receiver 参与规则。
...
方向规则：
- 方向也存储在升级物品 NBT（每个升级物品独立）。
- 已设置方向：仅该方向连接面参与。
- 未设置方向：视为任意面参与。
- 若 provider/receiver 一方设置方向、另一方未设置，则“未设置”的一方会自动排除该已设置方向（避免同一面同时入/出）。
...
- 升级驱动参与逻辑闭环（provider/receiver + NBT 过滤 + NBT 方向）。
- `SolarDistiller / GeoGenerator / Pump / OreWashingPlant` 接入。
