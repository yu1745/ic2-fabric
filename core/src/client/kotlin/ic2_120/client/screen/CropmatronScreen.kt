        ...
        val content: UiScope.() -> Unit = {
            Row(
                ...
                Column(spacing = 4, modifier = Modifier.EMPTY.width(GuiSize.STANDARD.contentWidth)) {
                    ...
                    Flex(direction = FlexDirection.ROW, justifyContent = JustifyContent.SPACE_BETWEEN, alignItems = AlignItems.END) {
                        ...
                        Column(spacing = 2) {
                            ...
                            SlotAnchor(id = slotAnchorId(CropmatronScreenHandler.SLOT_WEED_EX_INPUT_INDEX))
                            SlotAnchor(id = slotAnchorId(CropmatronScreenHandler.SLOT_WEED_EX_OUTPUT_INDEX))
                        }
                    }

                    Row(spacing = 2) {
                        SlotAnchor(id = slotAnchorId(CropmatronScreenHandler.SLOT_DISCHARGING_INDEX))
                        for (i in CropmatronScreenHandler.SLOT_FERTILIZER_INDEX_START..CropmatronScreenHandler.SLOT_FERTILIZER_INDEX_END) {
                ...
                Column(
                    ...
                    for (slotIndex in CropmatronScreenHandler.SLOT_UPGRADE_INDEX_START..CropmatronScreenHandler.SLOT_UPGRADE_INDEX_END) {
                        SlotAnchor(id = slotAnchorId(slotIndex), width = CropmatronScreenHandler.SLOT_SIZE, height = CropmatronScreenHandler.SLOT_SIZE)
                    }
                }
            }
