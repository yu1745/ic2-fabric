...
class CompressorScreen(
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        val progressFrac = if (CompressorSync.PROGRESS_MAX > 0) {
            (handler.sync.progress.coerceIn(0, CompressorSync.PROGRESS_MAX).toFloat() / CompressorSync.PROGRESS_MAX).coerceIn(0f, 1f)
        } else 0f
        val inputRate = handler.sync.getSyncedInsertedAmount()
        val consumeRate = handler.sync.getSyncedConsumedAmount()

        // 电量条 (180,4)-(194,18) = 14×14，自下而上
        ...
        if (relX in UPTIPS_X until UPTIPS_X + UPTIPS_SIZE &&
            ...
            context.drawTooltip(
                textRenderer,
                listOf(
                    Text.translatable("gui.ic2_120.compressor.uptips"),
    ...
    companion object {
        ...
        private const val PROGRESS_V = 22
