...
class PipeBlockEntityRenderer(
    ...
    override fun render(
        ...
        if (fluid != null) {
            ...
            if (fluidSprite != null) {
                // 核心（流体贴图）
                drawCuboidFluid(entry, fluidVc, light, overlay,
                ...
                for (dir in Direction.entries) {
                    ...
                    drawArmFluid(entry, fluidVc, light, overlay,
        ...
        drawWindowGlass(
            state, entry, glassVc, light, overlay, min, max, cutCenter,
    ...
    private fun drawContinuousCornerShell(
        ...
        entry: MatrixStack.Entry, vc: VertexConsumer,
    ...
    private fun drawPumpAttachmentPlate(
        ...
        val t = 2.0f / 16.0f
        when (facing) {
            Direction.NORTH -> drawCuboidColor(entry, vc, light, overlay, min, min, 0.0f, max, max, t, r, g, b, 255)
            Direction.SOUTH -> drawCuboidColor(entry, vc, light, overlay, min, min, 1.0f - t, max, max, 1.0f, r, g, b, 255)
            Direction.WEST -> drawCuboidColor(entry, vc, light, overlay, 0.0f, min, min, t, max, max, r, g, b, 255)
            Direction.EAST -> drawCuboidColor(entry, vc, light, overlay, 1.0f - t, min, min, 1.0f, max, max, r, g, b, 255)
            Direction.DOWN -> drawCuboidColor(entry, vc, light, overlay, min, 0.0f, min, max, t, max, r, g, b, 255)
            Direction.UP -> drawCuboidColor(entry, vc, light, overlay, min, 1.0f - t, min, max, 1.0f, max, r, g, b, 255)
        }
    }
    ...
    private fun drawGlassOnAxis(
        ...
        for ((segFrom, segTo) in segments) {
            ...
            drawGlassSegment(
                axis, segFrom, segTo, min, max, lo2, hi2, entry, vc, light, overlay,
    ...
    private fun drawGlassSegment(
        ...
        when (axis) {
            Direction.Axis.X -> {
                drawCuboidColor(entry, vc, light, overlay, from, min, lo2, to, min + t, hi2, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, from, max - t, lo2, to, max, hi2, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, from, lo2, min, to, hi2, min + t, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, from, lo2, max - t, to, hi2, max, paneR, paneG, paneB, paneA)
            }
            Direction.Axis.Y -> {
                drawCuboidColor(entry, vc, light, overlay, min, from, lo2, min + t, to, hi2, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, max - t, from, lo2, max, to, hi2, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, lo2, from, min, hi2, to, min + t, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, lo2, from, max - t, hi2, to, max, paneR, paneG, paneB, paneA)
            }
            Direction.Axis.Z -> {
                drawCuboidColor(entry, vc, light, overlay, min, lo2, from, min + t, hi2, to, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, max - t, lo2, from, max, hi2, to, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, lo2, min, from, hi2, min + t, to, paneR, paneG, paneB, paneA)
                drawCuboidColor(entry, vc, light, overlay, lo2, max - t, from, hi2, max, to, paneR, paneG, paneB, paneA)
            }
        }
    }
    ...
    private fun drawWindowGlassParticles(
        ...
        if (activeX || !hasAnyAxis) {
            drawParticlesOnAxis(
                ...
                entry, vc, light, overlay, particleR, particleG, particleB, particleA
            )
        }
        if (activeY || !hasAnyAxis) {
            drawParticlesOnAxis(
    ...
    private fun drawParticlesForGlassSegment(
        ...
        when (axis) {
            Direction.Axis.X -> {
                drawParticleSpotsOnPane(entry, vc, light, overlay, from, min, lo2, to, min + paneThickness, hi2, Direction.Axis.Y, density, particleR, particleG, particleB, particleA, 0x31A2)
                drawParticleSpotsOnPane(entry, vc, light, overlay, from, max - paneThickness, lo2, to, max, hi2, Direction.Axis.Y, density, particleR, particleG, particleB, particleA, 0x31A3)
                drawParticleSpotsOnPane(entry, vc, light, overlay, from, lo2, min, to, hi2, min + paneThickness, Direction.Axis.Z, density, particleR, particleG, particleB, particleA, 0x31A4)
                drawParticleSpotsOnPane(entry, vc, light, overlay, from, lo2, max - paneThickness, to, hi2, max, Direction.Axis.Z, density, particleR, particleG, particleB, particleA, 0x31A5)
            }
            Direction.Axis.Y -> {
                drawParticleSpotsOnPane(entry, vc, light, overlay, min, from, lo2, min + paneThickness, to, hi2, Direction.Axis.X, density, particleR, particleG, particleB, particleA, 0x42B2)
                drawParticleSpotsOnPane(entry, vc, light, overlay, max - paneThickness, from, lo2, max, to, hi2, Direction.Axis.X, density, particleR, particleG, particleB, particleA, 0x42B3)
                drawParticleSpotsOnPane(entry, vc, light, overlay, lo2, from, min, hi2, to, min + paneThickness, Direction.Axis.Z, density, particleR, particleG, particleB, particleA, 0x42B4)
                drawParticleSpotsOnPane(entry, vc, light, overlay, lo2, from, max - paneThickness, hi2, to, max, Direction.Axis.Z, density, particleR, particleG, particleB, particleA, 0x42B5)
            }
            Direction.Axis.Z -> {
                drawParticleSpotsOnPane(entry, vc, light, overlay, min, lo2, from, min + paneThickness, hi2, to, Direction.Axis.X, density, particleR, particleG, particleB, particleA, 0x53C2)
                drawParticleSpotsOnPane(entry, vc, light, overlay, max - paneThickness, lo2, from, max, hi2, to, Direction.Axis.X, density, particleR, particleG, particleB, particleA, 0x53C3)
                drawParticleSpotsOnPane(entry, vc, light, overlay, lo2, min, from, hi2, min + paneThickness, to, Direction.Axis.Y, density, particleR, particleG, particleB, particleA, 0x53C4)
                drawParticleSpotsOnPane(entry, vc, light, overlay, lo2, max - paneThickness, from, hi2, max, to, Direction.Axis.Y, density, particleR, particleG, particleB, particleA, 0x53C5)
            }
        }
    }
    ...
    private fun drawParticleSpotsOnPane(
        ...
        repeat(count) { i ->
            ...
            val a = (alpha * (GLASS_PARTICLE_ALPHA_VARIATION_MIN + hashToUnit(seed0 xor 0x998877) * alphaRange))
                .toInt()
                .coerceIn(8, 255)
            drawCuboidColor(entry, vc, light, overlay, x0, y0, z0, x1, y1, z1, r, g, b, a)
        }
    }
    ...
    private fun drawArmFluid(
        ...
        when (dir) {
            Direction.NORTH -> drawCuboidFluid(entry, vc, light, overlay, min, min, edgeMin, max, max, joinMin, sprite, color)
            Direction.SOUTH -> drawCuboidFluid(entry, vc, light, overlay, min, min, joinMax, max, max, edgeMax, sprite, color)
            Direction.WEST -> drawCuboidFluid(entry, vc, light, overlay, edgeMin, min, min, joinMin, max, max, sprite, color)
            Direction.EAST -> drawCuboidFluid(entry, vc, light, overlay, joinMax, min, min, edgeMax, max, max, sprite, color)
            Direction.DOWN -> drawCuboidFluid(entry, vc, light, overlay, min, edgeMin, min, max, joinMin, max, sprite, color)
            Direction.UP -> drawCuboidFluid(entry, vc, light, overlay, min, joinMax, min, max, edgeMax, max, sprite, color)
        }
    }
    ...
    private fun drawCuboidColor(
        ...
        // South (+Z)
        quadColor(vc, entry, light, overlay,
    ...
    private fun drawCuboidWindowFrame(
        ...
        if (fx <= 0.0f || fy <= 0.0f || fz <= 0.0f) {
            drawCuboidColor(entry, vc, light, overlay, minX, minY, minZ, maxX, maxY, maxZ, r, g, b, a)
            return
        }

        // South (+Z)
        drawFaceFrameXY(entry, vc, light, overlay, minX, minY, maxX, maxY, maxZ, fx, fy, 1f, r, g, b, a)
        // North (-Z)
        drawFaceFrameXY(entry, vc, light, overlay, minX, minY, maxX, maxY, minZ, fx, fy, -1f, r, g, b, a)
        // West (-X)
        drawFaceFrameYZ(entry, vc, light, overlay, minY, minZ, maxY, maxZ, minX, fy, fz, -1f, r, g, b, a)
        // East (+X)
        drawFaceFrameYZ(entry, vc, light, overlay, minY, minZ, maxY, maxZ, maxX, fy, fz, 1f, r, g, b, a)
        // Up (+Y)
        drawFaceFrameXZ(entry, vc, light, overlay, minX, minZ, maxX, maxZ, maxY, fx, fz, 1f, r, g, b, a)
        // Down (-Y)
        drawFaceFrameXZ(entry, vc, light, overlay, minX, minZ, maxX, maxZ, minY, fx, fz, -1f, r, g, b, a)
    }
    ...
    private fun drawCornerRibsOnAxis(
        ...
        when (axis) {
            Direction.Axis.X -> {
                drawCuboidColor(entry, vc, light, overlay, from, lo, lo, to, lo2, lo2, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, from, lo, hi2, to, lo2, hi, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, from, hi2, lo, to, hi, lo2, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, from, hi2, hi2, to, hi, hi, r, g, b, 255)
            }
            Direction.Axis.Y -> {
                drawCuboidColor(entry, vc, light, overlay, lo, from, lo, lo2, to, lo2, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, lo, from, hi2, lo2, to, hi, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, hi2, from, lo, hi, to, lo2, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, hi2, from, hi2, hi, to, hi, r, g, b, 255)
            }
            Direction.Axis.Z -> {
                drawCuboidColor(entry, vc, light, overlay, lo, lo, from, lo2, lo2, to, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, lo, hi2, from, lo2, hi, to, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, hi2, lo, from, hi, lo2, to, r, g, b, 255)
                drawCuboidColor(entry, vc, light, overlay, hi2, hi2, from, hi, hi, to, r, g, b, 255)
            }
        }
    }
    ...
    private fun drawCuboidFluid(
        ...
        // South (+Z)
        quadFluid(vc, entry, light, overlay,
    ...
    private fun quadColor(
        vc: VertexConsumer, entry: MatrixStack.Entry,
        light: Int, overlay: Int,
        ...
        vertexColor(vc, entry, x1, y1, z1, 0f, 0f, light, overlay, nx, ny, nz, r, g, b, a)
        vertexColor(vc, entry, x2, y2, z2, 1f, 0f, light, overlay, nx, ny, nz, r, g, b, a)
        vertexColor(vc, entry, x3, y3, z3, 1f, 1f, light, overlay, nx, ny, nz, r, g, b, a)
        vertexColor(vc, entry, x4, y4, z4, 0f, 1f, light, overlay, nx, ny, nz, r, g, b, a)
    }
    ...
    private fun quadFluid(
        ...
        vertexFluid(vc, entry, x1, y1, z1, u1, v1, light, overlay, nx, ny, nz, r, g, b)
        vertexFluid(vc, entry, x2, y2, z2, u2, v2, light, overlay, nx, ny, nz, r, g, b)
        vertexFluid(vc, entry, x3, y3, z3, u3, v3, light, overlay, nx, ny, nz, r, g, b)
        vertexFluid(vc, entry, x4, y4, z4, u4, v4, light, overlay, nx, ny, nz, r, g, b)
    }
