...
@ModScreenHandler(block = AnimalSlaughtererBlock::class)
class AnimalSlaughtererScreenHandler(
    ...
    companion object {
        ...
        @ScreenFactory
        fun fromBuffer(syncId: Int, playerInventory: PlayerInventory, buf: PacketByteBuf): AnimalSlaughtererScreenHandler {
            val pos = buf.readBlockPos()
            val propertyCount = buf.readVarInt()
            val context = ScreenHandlerContext.create(playerInventory.player.world, pos)
            val blockInv = SimpleInventory(AnimalSlaughtererBlockEntity.INVENTORY_SIZE)
            return AnimalSlaughtererScreenHandler(syncId, playerInventory, blockInv, context, ArrayPropertyDelegate(propertyCount))
        }
    }
}
